...
module wishbone_interconnect #(
    ...
    output wire [ADDR_WIDTH-MUX_ADDR_WIDTH-1:0] WBS_ADR,                  // Address bus for slaves
   ...
   // Generate block to expand the flattened arrays into 2D arrays
    ...
    always @(posedge WB_CLK or posedge WB_RST) begin
        if (WB_RST) begin
            ...
            active_slave <= 0;  // Reset active_slave each clock cycle
            WB_ACK <= 0;        // Default to no acknowledgment

            // Iterate over all slaves to check address match
            for (i = 0; i < NUM_SLAVES; i = i + 1) begin
                if (WB_ADR[ADDR_WIDTH-1:ADDR_WIDTH-MUX_ADDR_WIDTH-1] == SLAVE_BASE_ADDRESSES_EXPANDED[i][ADDR_WIDTH-1:ADDR_WIDTH-MUX_ADDR_WIDTH-1]) begin
                // if (WB_ADR[ADDR_WIDTH-1:MUX_ADDR_WIDTH-1] == SLAVE_BASE_ADDRESSES_EXPANDED[i][ADDR_WIDTH-1:MUX_ADDR_WIDTH-1]) begin
                    active_slave[i] <= 1;
    ...
    assign WBS_ADR = WB_ADR[ADDR_WIDTH-MUX_ADDR_WIDTH-1:0];
